...
export default function Page() {
	...
	async function fetchUserData(data: any) {
		const response = await fetch("http://localhost:3000/api/auth/register", {
			...
			body: JSON.stringify({ name: `${data.firstName} ${data.lastName}`, email: data.email, password: data.password, repeatPassword: data.confirmPassword }),
		})
		if (response.ok) {
			console.log('Ok')
		} else {
